...
def main():
    ...
    try:
        ...
        # Department selection (include "All" option)
        department_options = ["All"] + list(filtered_crew_df['department'].unique())
        ...
        if selected_department == "All":
            all_crew_ids = sorted(filtered_crew_df['crew_id'].unique())
        else:
        ...
        # Main content
        if selected_department == "All":
            # Individual crew member analysis (when "All" departments is selected)
            st.header(f"Individual Analysis - Crew ID: {selected_crew_id}")
            
            # Display crew member info
            crew_info = filtered_crew_df[filtered_crew_df['crew_id'] == selected_crew_id].iloc[0]
            ...
            crew_skills = get_crew_member_skills(filtered_crew_df, selected_crew_id)
            avg_skills = analyze_skill_gaps(filtered_crew_df)
            ...
            # Department-level analysis (when a specific department is selected)
            st.header(f"Department Analysis - {selected_department}")
            
            # Department Average Skills
            st.subheader("Department Average Skills")
            department_skill_gaps = analyze_skill_gaps(filtered_crew_df[filtered_crew_df['department'] == selected_department])
            ...
            department_training = training_df[training_df['crew_id'].isin(all_crew_ids)]
            ...
            department_feedback = feedback_df[feedback_df['crew_id'].isin(all_crew_ids)]
