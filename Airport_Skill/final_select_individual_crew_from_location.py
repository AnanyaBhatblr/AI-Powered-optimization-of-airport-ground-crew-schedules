...
def create_performance_visualization(performance_trends):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=performance_trends['task_completion_rate']*100,  # Already in percentage
...
# removed    crew_feedback['task_completion_rate'] = crew_feedback['task_completion_rate'] * 100    to convert it to percentage
...
def main():
    ...
    try:
        ...
        # Crew ID selection (only show crew IDs for the selected location)
        ...
        selected_department = st.sidebar.multiselect(
            ...
            default=filtered_crew_df['department'].unique()
        )
        
        # Filter data based on selected department
        filtered_crew_df = filtered_crew_df[filtered_crew_df['department'].isin(selected_department)]
        
        # Main content
        if not filtered_crew_df.empty:
            ...
            st.warning("No crew members found for the selected location and department.")
